...
from evaluation.slt_eval import external_metrics_sacrebleu
from evaluation.slt_eval import external_metrics_mscoco_rouge
import numpy as np
...
def chrf(references, hypotheses):
    ...
    """
    return (
        external_metrics_sacrebleu.corpus_chrf(hypotheses=hypotheses, references=references).score * 100
    )
...
def rouge(references, hypotheses):
    ...
    for h, r in zip(hypotheses, references):
        rouge_score += external_metrics_mscoco_rouge.calc_score(hypotheses=[h], references=[r]) / n_seq
