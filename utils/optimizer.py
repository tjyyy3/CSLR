...
class Optimizer(object):
    def __init__(self, model, optim_dict):
        ...
        elif self.optim_dict["optimizer"] in ['Plateau', 'Adam']:
    ...
    def define_lr_scheduler(self, optimizer, milestones):
        if self.optim_dict["optimizer"] in ['SGD', 'Adam']:
            ...
            return lr_scheduler
        elif self.optim_dict["optimizer"] in ['Plateau']:
            lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer, mode='min', verbose=False, threshold_mode="abs", factor=0.1, patience=5)
            return lr_scheduler
        else:
            raise ValueError()
